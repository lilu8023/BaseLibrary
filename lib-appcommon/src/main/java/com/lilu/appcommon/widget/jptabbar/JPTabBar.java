package com.lilu.appcommon.widget.jptabbar;
...
import com.lilu.appcommon.widget.jptabbar.animate.Animatable;
import com.lilu.appcommon.widget.jptabbar.animate.AnimationType;
import com.lilu.appcommon.widget.jptabbar.animate.FlipAnimater;
import com.lilu.appcommon.widget.jptabbar.animate.JumpAnimater;
import com.lilu.appcommon.widget.jptabbar.animate.RotateAnimater;
import com.lilu.appcommon.widget.jptabbar.animate.Scale2Animater;
import com.lilu.appcommon.widget.jptabbar.animate.ScaleAnimater;
import com.lilu.appcommon.widget.jptabbar.animate.ScaleAnimator;
...
public class JPTabBar extends LinearLayout implements ViewPager.OnPageChangeListener, View.OnTouchListener {
    ...
    private void init(Context context, AttributeSet set) {
        ...
        setMinimumHeight(DensityUtils.dp2px(context, 48));

        //获取所有节点属性
        mAttribute = context.obtainStyledAttributes(set, R.styleable.JPTabBar);
        normalColor = mAttribute.getColor(R.styleable.JPTabBar_TabNormalColor, DEFAULT_NORMAL_COLOR);
        selectColor = mAttribute.getColor(R.styleable.JPTabBar_TabSelectColor, DEFAULT_SELECT_COLOR);
        textSize = DensityUtils.px2sp(mContext, mAttribute.getDimensionPixelSize(R.styleable.JPTabBar_TabTextSize, DensityUtils.sp2px(mContext, DEFAULT_TEXTSIZE)));
        iconSize = mAttribute.getDimensionPixelSize(R.styleable.JPTabBar_TabIconSize, DensityUtils.dp2px(mContext, DEFAULT_ICONSIZE));
        margin = mAttribute.getDimensionPixelOffset(R.styleable.JPTabBar_TabMargin, DensityUtils.dp2px(mContext, DEFAULT_MARGIN));
        BadgeColor = mAttribute.getColor(R.styleable.JPTabBar_BadgeColor, DEFAULT_BADGE_COLOR);
        badgeTextSize = DensityUtils.px2sp(mContext, mAttribute.getDimensionPixelSize(R.styleable.JPTabBar_BadgeTextSize, DensityUtils.sp2px(mContext, DEFAULT_BADGE_TEXTSIZE)));
        badgePadding = DensityUtils.px2dp(mContext, mAttribute.getDimensionPixelOffset(R.styleable.JPTabBar_BadgePadding, DensityUtils.dp2px(mContext, DEFAULT_PADDING)));
        badgeVerMargin = DensityUtils.px2dp(mContext, mAttribute.getDimensionPixelOffset(R.styleable.JPTabBar_BadgeVerticalMargin, DensityUtils.dp2px(mContext, DEFAULT_BADGEVERTICAL_MARGIN)));
        badgeHorMargin = DensityUtils.px2dp(mContext, mAttribute.getDimensionPixelOffset(R.styleable.JPTabBar_BadgeHorizonalMargin, DensityUtils.dp2px(mContext, DEFAULT_BADGEHORIZONAL_MARGIN)));
        ...
        hMargin = mAttribute.getDimensionPixelOffset(R.styleable.JPTabBar_TabMiddleHMargin, DensityUtils.dp2px(mContext, DEFAULT_MIDDLEMARGIN));
        typeFacePath = mAttribute.getString(R.styleable.JPTabBar_TabTypeface);
        acceptFilter = mAttribute.getBoolean(R.styleable.JPTabBar_TabIconFilter, DEFAULT_ACEEPTFILTER);
        tabSelectBg = mAttribute.getDrawable(R.styleable.JPTabBar_TabSelectBg);
        hasMind = mAttribute.getResourceId(R.styleable.JPTabBar_TabMiddleView, 0);
    ...
    public void initFromAttribute() {
        ...
        if (!isInEditMode()) {
            ...
            for (int i = 0; i < mJPTabItems.length; i++) {
                ...
                Animatable animater = new ScaleAnimator();

                mJPTabItems[i] = new JPTabItem.Builder(mContext)
                        .setTitle(mTitles == null ? null : mTitles[i])
                        .setIndex(temp)
                        .setTextSize(textSize)
                        .setTypeFacePath(typeFacePath)
                        .setNormalColor(normalColor)
                        .setSelectBg(tabSelectBg)
                        .setBadgeColor(BadgeColor)
                        .setBadgeTextSize(badgeTextSize)
                        .setNormalIcon(mNormalIcons[i])
                        .setSelectedColor(selectColor)
                        .setBadgeHorMargin(badgeHorMargin)
                        .setBadgePadding(badgePadding)
                        .setIconSize(iconSize)
                        .setIconFilte(acceptFilter)
                        .setBadgeVerMargin(badgeVerMargin)
                        .setMargin(margin)
                        .setAnimater(animater)
                        .setSelectIcon(mSelectedIcons == null ? 0 : mSelectedIcons[i])
                        .build();
                ...
                if (i == (mJPTabItems.length / 2 - 1) && hasMind != 0) {
                    //添加中间的占位距离控件
                    View mind_view = new View(mContext);
                    ViewGroup.LayoutParams params = new ViewGroup.LayoutParams(hMargin, ViewGroup.LayoutParams.MATCH_PARENT);
                    mind_view.setLayoutParams(params);
                    addView(mind_view);
                }
            }
